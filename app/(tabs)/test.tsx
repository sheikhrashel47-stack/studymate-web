...
export default function TestTab() {
  ...
  return <StudyScreen><AppHeader title="Test" subtitle="Choose a focused practice mode" /><View style={styles.content}>{hasQuestions ? <><ModeCard title="Mock Test" detail="Practice like a real exam, then review your answers." icon="edit-note" action="Set up test" onPress={() => router.push("/test/setup")} /><ModeCard title="Flash Test" detail="Choose a topic and get instant feedback." icon="bolt" action="Set up Flash Test" onPress={() => router.push("/test/flash-setup")} /></> : <EmptyState title="Add questions first" detail="Import questions to create your mock tests and flash practice." action={<PrimaryButton label="Import Questions" icon="file-upload" onPress={() => router.push("/questions/import")} />} />}</View></StudyScreen>;
}
