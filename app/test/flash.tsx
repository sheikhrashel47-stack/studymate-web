...
export default function FlashTestScreen() {
  ...
  useEffect(() => { if (finishedRef.current || !questionIds.length || !durationSeconds || remaining > 0) return; finishedRef.current = true; const result = completeFlashTest(questionIds, answers, startedAt, 0, durationSeconds); if (result) router.replace({ pathname: "/test/result", params: { testId: result.id, timedOut: "1" } }); }, [answers, completeFlashTest, durationSeconds, questionIds, remaining, startedAt]);
  ...
  const next = () => { if (finishedRef.current) return; if (index >= questions.length - 1) { finishedRef.current = true; const result = completeFlashTest(questionIds, answers, startedAt, remaining, durationSeconds); if (result) router.replace({ pathname: "/test/result", params: { testId: result.id } }); return; } setIndex((value) => value + 1); setSelected(answers[questionIds[index + 1]]); };
